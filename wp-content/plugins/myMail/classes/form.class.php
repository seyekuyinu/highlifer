...
class mymail_form {
	...
	public function render($echo = true) {
		...
		if($this->honeypot){
			...
			array('_honeypot' => '<label style="position:absolute;top:-99999px;'.(is_rtl() ? 'right' : 'left').':-99999px;z-index:-99;"><input name="n_'.wp_create_nonce( 'honeypot' ).'_email" type="email" tabindex="-1"></label>') +
		...
		$html = str_replace('%%FORMACTION%%', $this->get_form_action('mymail_form_submit'), $html);
	...
	public function update( ) {
		...
		foreach ($form->fields as $field){
			...
			$value = esc_attr(isset($_POST['mymail_'.$field->field_id])
						? $_POST['mymail_'.$field->field_id]
						: (isset($_POST['userdata'][$field->field_id]) ? $_POST['userdata'][$field->field_id] : ''));
	...
	static function print_style($embed = true) {
		...
		($embed)
			? mymail('helper')->wp_print_embedded_styles('mymail-form-default', !empty($suffix))

...
class mymail_lists {
	...
	public function view_lists( ) {
		...
		if(isset($_GET['ID']) || isset($_GET['new'])) :
			...
			wp_enqueue_script('easy-pie-chart', MYMAIL_URI . 'assets/js/easy-pie-chart'.$suffix.'.js', array('jquery'), MYMAIL_VERSION);
	...
	public function merge($ids, $newname = NULL) {
		...
		if(!is_wp_error( $new_id )){
			...
			//move connections
			$sql = "UPDATE {$wpdb->prefix}mymail_lists_subscribers SET list_id = %d, added = %d WHERE list_id IN (".implode(', ', array_filter($ids, 'is_numeric')).")";

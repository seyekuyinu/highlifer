...
class mymail_templates {
			...
			foreach($folders as $folder){
				...
				if(file_exists($uploadfolder.'/'.$folder.'/index.html')){
					...
					if(true){
						...
						$zips = glob($uploadfolder.'/*mymail*.zip');
						foreach ($zips as $zip) {
	...
	public function scripts_styles() {
		...
		wp_enqueue_script('mymail-codemirror', MYMAIL_URI . 'assets/js/codemirror'.$suffix.'.js', array(), MYMAIL_VERSION);
			...
			if(isset($_GET['mymail_error'])){
				...
				switch ($error) {
					case 'Could not find item with provided item id or purchase code':
						...
						$error .= ' <a href="'.$template['uri'].'">'.__("get this template", 'mymail').'</a>';
						break;
					default:
	...
	public function download_http_request_args($r, $url) {
		...
		$wp_header_to_desc[681] = 'Licensecode expired!';

...
if ( ! class_exists( 'MPC_Quote' ) ) {
	class MPC_Quote {
		...
		function shortcode_template( $atts, $description = null ) {
			...
			/* Layout parts */
			$this->parts[ 'section_begin' ] = '<blockquote class="mpc-quote__content' . $quote_classes . '">';
			$this->parts[ 'section_end' ]   = '</blockquote>';
			$this->parts[ 'description' ]   = '<div class="mpc-quote__description">' . wpb_js_remove_wpautop( $description, true ) . '</div>';
			$this->parts[ 'signature' ]     = '<footer class="mpc-quote__signature' . $author_classes . '"><cite>' . $atts[ 'author' ] . '</cite></footer>';
		...
		function shortcode_styles( $styles ) {
			...
			// Content
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'quote' ) ) {
			...
			// Author
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'author' ) ) {

	...
	class MPC_Carousel_Anything {
		...
		function shortcode_styles( $styles ) {
			...
			if ( $styles[ 'gap' ] && $styles[ 'gap' ] != '0px' ) {
				$style .= '.mpc-carousel-anything[id="' . $css_id . '"] .slick-list {';
			...
			// Item
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'item_odd' ) ) {
				$style .= '.mpc-carousel-anything[id="' . $css_id . '"] .mpc-carousel__item-wrapper.slick-slide:nth-child(2n+1) {';
					$style .= $temp_style;
				$style .= '}';
			}

			if ( $temp_style = MPC_Snippets::css_background( $styles, 'item_even' ) ) {

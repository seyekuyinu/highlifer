...
if ( ! class_exists( 'MPC_Single_Post' ) ) {
	class MPC_Single_Post {
		...
		function __construct() {
			...
			add_filter( 'vc_autocomplete_mpc_single_post_taxonomies_callback', 'vc_autocomplete_taxonomies_field_search', 10, 1 );
			add_filter( 'vc_autocomplete_mpc_single_post_taxonomies_render', 'vc_autocomplete_taxonomies_field_render', 10, 1 );
		...
		/* Get Tax list */
		function get_taxonomies( $layout ) {
			$separator = ( $layout == 'style_1' || $layout == 'style_4' || $layout == 'style_7' ) ? __( ' / ', 'mpc' ) : __( ', ', 'mpc' );
		...
		function get_icon( $atts, $type = 'lightbox', $css_id = '' ) {
			$icon = '';
		...
		function shortcode_layout( $style ) {
			...
			$layouts = array(
				...
				'style_5' => array( 'thumbnail', 'overlay', 'section_begin', 'title', 'description', 'readmore', 'meta', 'section_end' ),
		...
		function item_template( $atts, $css_id = '' ) {
			global $MPC_Button;

			$atts = shortcode_atts( $this->defaults, $atts );
			$atts_lightbox = MPC_Snippets::parse_atts_icon( $atts, 'lightbox' );
			$atts_readmore = MPC_Snippets::parse_atts_icon( $atts, 'readmore' );
			$atts_button   = MPC_Snippets::parse_atts_shortcode( $atts, 'mpc_button_' );
			...
			$atts_button[ 'url' ]   = 'url:' . apply_filters( 'ma/single_post/permalink', get_the_permalink(), 'permalink' ) . '|title:' . apply_filters( 'ma/single_post/title', get_the_title(), 'title' ) . '|';
			...
			$this->parts[ 'taxonomies' ]  = isset( $meta_layout[ 'taxonomies' ] ) ? '<span class="mpc-post__tax">' . $this->get_taxonomies( $atts[ 'layout' ] ) . '</span>' : '';
			...
			$this->parts[ 'thumbnail' ]   = '<div class="mpc-post__thumbnail"' . $this->get_thumbnail() . '>' . $this->thumbnail_layout( $atts[ 'layout' ] ) . '</div>';
			...
			$this->parts[ 'lightbox_icon' ] = $atts[ 'lightbox_disable' ] == '' ? $this->get_icon( $atts_lightbox, 'lightbox', $css_id ) : '';
			$this->parts[ 'readmore_icon' ] = $atts[ 'readmore_disable' ] == '' ? $this->get_icon( $atts_readmore, 'readmore', $css_id ) : '';
			...
			$return = '<div class="mpc-template" data-template="mpc-post">';
		...
		function shortcode_template( $atts, $content = null, $shortcode = null, $parent_css = null ) {
			...
			while( $this->query->have_posts() ) {
				...
				if( ! $this->is_wrapped ) {
					$return .= '<div' . $css_id . ' class="mpc-single-post' . esc_attr( $this->classes ) .'"' . $animation . '>';
				}
		...
		function shortcode_styles( $styles, $parent_css ) {
			...
			// Regular
			if ( $temp_style = MPC_Snippets::css_background( $styles ) ) {
			...
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'date' ) ) { $inner_styles[] = $temp_style; }
			...
			if( $temp_style = MPC_Snippets::css_font( $styles, 'title' ) ) { $inner_styles[] = $temp_style; }
			...
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'description' ) ) { $inner_styles[] = $temp_style; }
			...
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'meta' ) ) { $inner_styles[] = $temp_style; }
			...
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'lightbox' ) ) {
			...
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'readmore' ) ) {

...
if ( ! class_exists( 'MPC_Icon_List' ) ) {
	class MPC_Icon_List {
		...
		function shortcode_template( $atts, $content = null ) {
			...
			for( $i=0; $i < count( $list_items ); $i++ ) {
				...
				$return .= '<li class="mpc-list__item mpc-parent-hover mpc-transition" data-index="' . $i . '">';
					$return .= $icon;
					$return .= isset( $item[ 'title' ] ) && $item[ 'title' ] != '' ? '<span class="mpc-list__title">' . $item[ 'title' ] . '</span>' : '';
				$return .= '</li>';
			}
		...
		function shortcode_styles( $styles, $list_items ) {
			...
			if ( $temp = MPC_Snippets::css_font( $styles ) ) { $inner_styles[] = $temp; }
			if ( $temp = MPC_Snippets::css_background( $styles ) ) { $inner_styles[] = $temp; }

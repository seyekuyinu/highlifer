...
if ( ! class_exists( 'MPC_Button' ) ) {
	class MPC_Button {
		...
		function shortcode_template( $atts, $content = null ) {
			...
			$url_settings =  $mpc_can_link ? MPC_Snippets::parse_atts_url( $atts[ 'url' ] ) : '';
			$wrapper = $url_settings != '' ? 'a' : 'div';

			$icon = MPC_Snippets::parse_atts_icon( $atts );
			...
			$atts_tooltip = MPC_Snippets::parse_atts_shortcode( $atts, 'mpc_tooltip_' );
			...
			$return = $tooltip != '' ? '<div class="mpc-tooltip-wrap" data-id="' . $css_id . '">' : '';
				$return .= '<' . $wrapper . $url_settings . ' data-id="' . $css_id . '" class="mpc-button' . $classes . '" ' . $animation . '>';
		...
		function shortcode_styles( $styles ) {
			...
			if ( count( $inner_styles ) > 0 ) {
				$style .= '.mpc-button[data-id="' . $css_id . '"] {';
			...
			if ( count( $inner_styles ) > 0 ) {
				$style .= '.mpc-button[data-id="' . $css_id . '"] .mpc-button__icon {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			if ( $styles[ 'padding_css' ] ) {
				$style .= '.mpc-button[data-id="' . $css_id . '"] .mpc-button__title {';
					$style .= $styles[ 'padding_css' ];
				$style .= '}';
			}

			if ( $temp_style = MPC_Snippets::css_background( $styles, 'hover' ) ) {
				$style .= '.mpc-button[data-id="' . $css_id . '"] .mpc-button__background {';
					$style .= $temp_style;
				$style .= '}';
			}
			...
			if ( $styles[ 'hover_border_css' ] ) {
				$style .= '.mpc-button[data-id="' . $css_id . '"]:hover,';
			...
			if ( $styles[ 'hover_icon_color' ] ) {
				$style .= '.mpc-button[data-id="' . $css_id . '"]:hover .mpc-button__icon,';
			...
			if ( $styles[ 'hover_font_color' ] ) {
				$style .= '.mpc-button[data-id="' . $css_id . '"]:hover .mpc-button__title,';
			...
			if ( $styles[ 'hover_background_offset' ] ) {
				...
				if ( $temp_style ) {
					$style .= '.mpc-button[data-id="' . $css_id . '"]:hover .mpc-button__background,';

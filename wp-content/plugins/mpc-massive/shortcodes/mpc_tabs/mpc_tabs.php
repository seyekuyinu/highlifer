...
if ( ! class_exists( 'MPC_Tab' ) ) {
	class MPC_Tab {
		...
		function shortcode_template( $atts, $description = null ) {
			...
			$tab_atts .= $MPC_Shortcode[ 'tabs' ][ 'nav' ] == '' ? ' data-active="true"' : ' data-active="false"';

			$nav_classes = $MPC_Shortcode[ 'tabs' ][ 'nav' ] == '' ? ' mpc-active' : '';
			$nav_atts = $atts[ 'tab_id' ] != '' ? ' data-tab_id="' . esc_attr( $atts[ 'tab_id' ] ) . '"' : '';

			/* Merge Buttons options */
			$button_atts = MPC_Snippets::parse_atts_shortcode( $atts, 'mpc_button_' );
...
if ( ! class_exists( 'MPC_Tabs' ) ) {
	...
	class MPC_Tabs {
		...
		function shortcode_template( $atts, $description = null ) {
			...
			MPC_Snippets::add_typography( $atts[ 'font_preset' ] );

			$animation = MPC_Snippets::parse_atts_animation( $atts );
			$MPC_Shortcode[ 'tabs' ][ 'button' ] = MPC_Snippets::parse_atts_shortcode( $atts, 'mpc_button_' );
			...
			$this->parts[ 'navigation'] = '<div class="mpc-tabs__nav"><ul>' . $MPC_Shortcode[ 'tabs' ][ 'nav' ] . '</ul></div>';

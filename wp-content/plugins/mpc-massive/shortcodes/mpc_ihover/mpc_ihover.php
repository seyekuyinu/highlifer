...
if ( ! class_exists( 'MPC_iHover' ) ) {
	class MPC_iHover {
		...
		function shortcode_styles( $styles ) {
			...
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'image' ) ) { $inner_styles[] = $temp_style; }
			...
			// Content
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'content' ) ) {
			...
			// Masks
			if ( $styles[ 'effect' ] == 'effect4' && $styles[ 'shape' ] == 'square' && $temp_style = MPC_Snippets::css_background( $styles, 'content' ) ) {

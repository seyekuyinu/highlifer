...
if ( ! class_exists( 'MPC_Snippets' ) ) {
	class MPC_Snippets {
		...
		/*----------------------------------------------------------------------------*\
			HELPERS
		\*----------------------------------------------------------------------------*/
		static function parse_atts_url( $url, &$url_title = '' ) {
			global $mpc_can_link;
			...
			if( filter_var( $url, FILTER_VALIDATE_URL ) !== FALSE && $mpc_can_link ) {
				...
				if ( isset( $link[ 'url' ] ) ) {
					...
					$settings  = ' href="' . esc_url( $link[ 'url' ] ). '" target="' . esc_attr( $link[ 'target' ] ) . '" title="' . esc_attr( $link[ 'title' ] ) . '" ';
			...
			if ( $settings[ $prefix . 'icon_type' ] == 'icon' && $settings[ $prefix . 'icon' ] != '' ) {
				...
				if ( $settings[ $prefix . 'icon_image' ] != '' ) {
					$icon_image = wpb_getImageBySize( array(
						...
						'thumb_size' => $settings[ $prefix . 'icon_image_size' ]
					) );
		...
		static function parse_atts_animation( $settings ) {
			$animation = '';

			if ( isset( $settings[ 'animation_in_type' ] ) && $settings[ 'animation_in_type' ] != 'none' ) {
			...
			if ( isset( $settings[ 'animation_loop_type' ] ) && $settings[ 'animation_loop_type' ] != 'none' ) {
		...
		static function parse_atts_carousel( $settings, $prefix = '', $custom = array() ) {
			...
			$atts = ! empty( $atts ) ? " data-mpcslick='" . json_encode( $atts ) . "'" . ' data-columns="' . esc_attr( $atts[ 'slidesToShow' ] ) . '"' : '';

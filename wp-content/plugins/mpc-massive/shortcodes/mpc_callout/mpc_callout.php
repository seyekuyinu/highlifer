...
if ( ! class_exists( 'MPC_Callout' ) ) {
	class MPC_Callout {
		...
		function shortcode_template( $atts, $description = null ) {
			...
			$atts_divider = MPC_Snippets::parse_atts_shortcode( $atts, 'mpc_divider_' );
			$atts_icon    = MPC_Snippets::parse_atts_icon( $atts );
		...
		function shortcode_styles( $styles ) {
			...
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'content' ) ) { $inner_styles[] = $temp_style; }

...
if ( ! class_exists( 'MPC_Navigation' ) ) {
	class MPC_Navigation {
		...
		function shortcode_template( $name, $content = null, $css_id = '', $type = '', $carousel = '' ) {
			...
			$previous_icon = MPC_Snippets::parse_atts_icon( $atts, 'previous' );
			$next_icon     = MPC_Snippets::parse_atts_icon( $atts, 'next' );
		...
		function shortcode_styles( $styles, $preset ) {
			...
			$inner_styles = array();
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'previous' ) ) { $inner_styles[] = $temp_style; }
			if ( count( $inner_styles ) > 0 ) {
			...
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'next' ) ) {

...
if ( ! class_exists( 'MPC_Image' ) ) {
	class MPC_Image {
		...
		function shortcode_template( $atts, $content = null ) {
			...
			$overlay   = MPC_Snippets::render_overlay( $atts, $image_full, $atts[ 'image_link' ] );
			$classes   .= $overlay[ 'class' ];
			$data_atts .= $overlay[ 'atts' ];

			/* Image Stuff */
			$url_settings = $mpc_can_link && $overlay[ 'content' ] == '' ? MPC_Snippets::parse_atts_url( $atts[ 'image_link' ] ) : '';

	...
	class MPC_WC_Add_To_Cart {
		...
		function get_title( $product, $type, $state = '' ) {
			switch( $type ) {
				...
				case 'price' : return $product->get_price_html(); break;
				case 'title' : return $product->get_title(); break;
		...
		function prepare_notices_atts() {
			...
			foreach( $icons_defualts as $notice => $values ) {
		...
		function loader_notice() {
			$loader_icon  = MPC_Snippets::parse_atts_icon( $this->atts, 'loader' );
		...
		function notices_markup() {
			$success_icon = MPC_Snippets::parse_atts_icon( $this->atts, 'success' );
			$error_icon   = MPC_Snippets::parse_atts_icon( $this->atts, 'error' );
		...
		function shortcode_template( $atts, $content = null ) {
			...
			if( $data_cart == '' ) {
				$url_settings =  $mpc_can_link ? MPC_Snippets::parse_atts_url( 'url:' . urlencode( $product->add_to_cart_url() ). '|title:' . $product->add_to_cart_text() . '|' ) : '';
			...
			$icon       = MPC_Snippets::parse_atts_icon( $this->atts );
			$hover_icon = MPC_Snippets::parse_atts_icon( $this->atts, 'hover' );
			...
			$styles = $this->shortcode_styles( $this->atts );
			$css_id = $styles[ 'id' ];

			$atts_tooltip = MPC_Snippets::parse_atts_shortcode( $this->atts, 'mpc_tooltip_' );
			$tooltip      = $this->atts[ 'mpc_tooltip__disable' ] == '' ? $MPC_Tooltip->shortcode_template( $atts_tooltip ) : '';

			$animation = MPC_Snippets::parse_atts_animation( $this->atts );
			...
			$classes   .= $this->atts[ 'block' ] != '' ? ' mpc-display--block' : '';
			...
			$return .= $this->atts[ 'loader_position' ] == 'outside' ? '<div class="mpc-wc-add_to_cart-wrap ' . $classes_wrap .'">' : '';
				$return .= '<' . $wrapper . $url_settings . $data_cart . ' data-id="' . $css_id . '" class="mpc-wc-add_to_cart' . $classes . '" ' . $animation . '>';
					$return .= '<div class="mpc-atc__content ' . $hover_effect_class . '">';

						$return .= '<span class="mpc-atc__title mpc-transition">' . $this->get_title_markup( $product, $icon ) . '</span>';
						...
						if( $this->atts[ 'hover_title_type' ] == 'none' ) {
							$return .= '<span class="mpc-atc__title-hover mpc-transition">' . $this->get_title_markup( $product, $icon ) . '</span>';
						} else {
							$return .= '<span class="mpc-atc__title-hover mpc-transition">' . $this->get_title_markup( $product, $hover_icon, 'hover_' ). '</span>';
						}
					...
					$return .= '</div>';
					$return .= '</' . $wrapper . '>';
				$return .= $this->atts[ 'loader_position' ] == 'outside' ? $this->loader_notice() . '</div>' : '';
				$return .= $tooltip;
			$return .= $tooltip != '' ? '</div>' : '';
		...
		function shortcode_styles( $styles ) {
			...
			if ( count( $inner_styles ) > 0 ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] {';
			...
			if ( $temp_style = MPC_Snippets::css_icon( $styles ) ) { $inner_styles[] = $temp_style; }
			if ( count( $inner_styles ) > 0 ) {
				$style .= $styles[ 'hover_title_type' ] == 'none' ? '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__title-hover .mpc-atc__icon,' : '';
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__title .mpc-atc__icon {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			if ( $styles[ 'padding_css' ] ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__title,';
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__title-hover {';
//				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__notice {';
					$style .= $styles[ 'padding_css' ];
				$style .= '}';
			}

			if ( $temp_style = MPC_Snippets::css_background( $styles, 'hover' ) ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__background {';
					$style .= $temp_style;
				$style .= '}';
			}
			...
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'hover' ) ) { $inner_styles[] = $temp_style; }
			if ( $styles[ 'hover_title_type' ] != 'none' && count( $inner_styles ) > 0 ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__title-hover .mpc-atc__icon {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			if ( $styles[ 'hover_border_css' ] ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"]:hover {';
					$style .= $styles[ 'hover_border_css' ];
				$style .= '}';
			}

			if ( $temp_style = MPC_Snippets::css_font( $styles, 'hover' ) ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc-atc__title-hover {';
					$style .= $temp_style;
				$style .= '}';
			}
			...
			if ( $styles[ 'hover_background_offset' ] ) {
				...
				if ( $temp_style ) {
					$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"]:hover .mpc-atc__background {';
						$style .= $temp_style;
					$style .= '}';
				}
			}
			...
			/* Notices */
			$inner_styles = array();
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'loader' ) ) { $inner_styles[] = $temp_style; }
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'loader' ) ) { $inner_styles[] = $temp_style; }
			if ( count( $inner_styles ) > 0 ) {
				$style .= $this->atts[ 'loader_position' ] == 'inside' ? '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc--loader {' : '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] + .mpc--loader {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			$inner_styles = array();
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'success' ) ) { $inner_styles[] = $temp_style; }
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'success' ) ) { $inner_styles[] = $temp_style; }
			if ( count( $inner_styles ) > 0 ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc--success {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			$inner_styles = array();
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'error' ) ) { $inner_styles[] = $temp_style; }
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'error' ) ) { $inner_styles[] = $temp_style; }
			if ( count( $inner_styles ) > 0 ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"] .mpc--error {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			if ( $styles[ 'loader_border' ] && $this->atts[ 'loader_position' ] == 'inside' ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"][data-notice$="loader"] {';
					$style .= 'border-color:' . $styles[ 'loader_border' ] . ';';
				$style .= '}';
			}
			if ( $styles[ 'loader_border' ] && $this->atts[ 'loader_position' ] == 'outside' ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"][data-notice$="loader"] + .mpc--loader {';
			...
			if ( $styles[ 'success_border' ] ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"][data-notice$="success"] {';
					$style .= 'border-color:' . $styles[ 'success_border' ] . ';';
				$style .= '}';
			}
			if ( $styles[ 'error_border' ] ) {
				$style .= '.mpc-wc-add_to_cart[data-id="' . $css_id . '"][data-notice$="error"] {';
...
//					'dependency'       => array( 'element' => 'hover_title_type', 'value_not_equal_to' => array( 'icon', 'price', 'title', 'custom' ) ),
			...
			$font       = MPC_Snippets::vc_font( array( 'dependency' => array( 'element' => 'title_type', 'value_not_equal_to' => 'icon' ) ) );

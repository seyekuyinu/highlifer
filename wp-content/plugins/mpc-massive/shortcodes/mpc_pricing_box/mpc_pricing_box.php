	...
	class MPC_Pricing_Box {
		...
		function shortcode_template( $atts, $content = null ) {
			...
			$MPC_Shortcode[ 'pricing' ] = array(
				...
				'parts' => array(
					'legend'  => '',
					'columns' => '',
					'button'  => MPC_Snippets::parse_atts_shortcode( $atts, 'mpc_button_' ),
				),
				'style' => '',
			);
			...
			$columns = $atts[ 'cols' ] != '' ? ' data-columns="' . esc_attr( $atts[ 'cols' ] ) . '"' : '';
			...
			$carousel_atts  = $atts[ 'slider_disable' ] != 'true' ? MPC_Snippets::parse_atts_carousel( $atts, '', array( 'edgeFriction' => 0, 'loop' => 'false' ) ) : '';
		...
		function shortcode_styles( $styles, $child_styles = '' ) {
			...
			if ( $temp_style = MPC_Snippets::css_background( $styles ) ) {
				$style .= '.mpc-pricing-box[id="' . $css_id . '"] .mpc-pricing-column--legend {';
					$style .= $temp_style;
				$style .= '}';
			}
			...
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'price' ) ) { $inner_styles[] = $temp_style; }
			...
			// Properties
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'prop' ) ) {

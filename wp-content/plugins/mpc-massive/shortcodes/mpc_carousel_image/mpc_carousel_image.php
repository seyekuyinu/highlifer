...
if ( ! class_exists( 'MPC_Carousel_Image' ) ) {
	class MPC_Carousel_Image {
		...
		function shortcode_template( $atts, $content = null ) {
			...
			$carousel  = MPC_Snippets::parse_atts_carousel( $atts );
			...
			if ( $atts[ 'overlay_enable_lightbox' ] != '' ) {
				...
				$overlay_icon = MPC_Snippets::parse_atts_icon( $atts, 'overlay' );
			...
			if ( $atts[ 'overlay_enable_url' ] != '' ) {
				...
				$url_icon = MPC_Snippets::parse_atts_icon( $atts, 'overlay_url' );
		...
		function shortcode_styles( $styles ) {
			...
			if ( $styles[ 'gap' ] && $styles[ 'gap' ] != '0px' ) {
				$style .= '.mpc-carousel-image[id="' . $css_id . '"] .slick-list {';
			...
			// Regular
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'image_odd' ) ) {
			...
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'image_even' ) ) {
			...
			// Hover
			if ( $temp_style = MPC_Snippets::css_background( $styles, 'image_hover' ) ) {
			...
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'overlay_url' ) ) {
				$style .= '.mpc-carousel-image[id="' . $css_id . '"] .mpc-type--external {';
					$style .= $temp_style;
				$style .= '}';
			}
			if ( $temp_style = MPC_Snippets::css_icon( $styles, 'overlay' ) ) {

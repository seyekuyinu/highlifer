...
if ( ! class_exists( 'MPC_Testimonial' ) ) {
	class MPC_Testimonial {
		...
		function shortcode_template( $atts, $description = null ) {
			...
			$url_title = '';
			$url       = MPC_Snippets::parse_atts_url( $atts[ 'url' ], $url_title );
			...
			$signature = '<footer class="mpc-testimonial__signature' . $author_classes . '">';
				$signature .= '<cite>' . $atts[ 'author' ] . '</cite>';
				$signature .= $url ? ' <a class="mpc-testimonial__link' . $url_classes . '" ' . $url . '>' . $url_title . '</a>' : '';
			$signature .= '</footer>';

			$this->parts[ 'section_begin' ] = '<blockquote class="mpc-testimonial__content' . $testimonial_classes . '">';
		...
		function shortcode_styles( $styles ) {
			...
			// Description
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'testimonial' ) ) {
			...
			// URL
			if ( $temp_style = MPC_Snippets::css_font( $styles, 'url' ) ) {

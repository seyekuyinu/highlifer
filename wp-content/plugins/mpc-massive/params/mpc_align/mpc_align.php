...
vc_add_shortcode_param( 'mpc_align', 'mpc_align_settings', MPC_MASSIVE_URL . '/assets/js/mpc-params.js' );
function mpc_align_settings( $settings, $value ) {
	...
	foreach ( $grid as $grid_item ) {
		...
		$align .= '<input id="' . esc_attr( $settings[ 'param_name' ] . '-radio_' . $grid_item ) . '" class="mpc-align-radio" name="' . esc_attr( $settings[ 'param_name' ] ) . '-radio" type="radio" ' . checked( $grid_item, $value ) . ' value="' . esc_attr( $grid_item ) . '" />';

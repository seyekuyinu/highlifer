...
/* Load preset */
	...
	function update_all_fields( $fields, values, _prefix ) {
		...
		$fields.each( function() {
			...
			if ( $parent.is( '.wpb_el_type_checkbox, .wpb_el_type_textarea_raw_html' ) ) {
...
/* Save preset */
	function save_preset( $preset, $fields, _shortcode, _prefix, _wp_nonce ) {
		if ( $preset.val() != '' ) {
			if ( confirm( _mpc_lang.mpc_preset.save_confirm + '"' + $preset.find( ':selected' ).text() + '"' ) ) {
				var _values = get_fields_values( $fields.filter( ':not(.mpc-ignored-field)' ), _prefix );
...
/* Delete preset */
	function delete_preset( $preset, _shortcode, _wp_nonce ) {
		if ( $preset.val() != '' ) {
			if ( confirm( _mpc_lang.mpc_preset.delete_confirm + '"' + $preset.find( ':selected' ).text() + '"' ) ) {
				set_ajax_state( $preset, true );
...
/* Preset field */
	...
	$presets.each( function() {
		var $preset                 = $( this ),
			...
			$fields                 = $preset.parents( '.vc_edit_form_elements' ).find( 'input, textarea, select' ).not( $preset ).filter( '.wpb_vc_param_value' ),

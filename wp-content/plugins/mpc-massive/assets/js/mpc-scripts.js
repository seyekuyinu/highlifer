...
(function( $ ) {
	...
	function draw_connections( $icons ) {
		var $line        = $icons.find( '.mpc-connected-icons__line' ),
		    ...
		    $target      = _target == 'box' ? $icons.find( '.mpc-icon-column' ) : $icons.find( '.mpc-icon' ),
...
( function( $ ) {
	...
	function init_shortcode( $grid_images ) {
		var $row = $grid_images.parents( '.vc_row' );
...
( function( $ ) {
	...
	function init_shortcode( $grid_posts ) {
		var $row = $grid_posts.parents( '.vc_row' );
...
( function( $ ) {
	...
	function calculate_navigation( $navigation, $carousel ) {
		...
		if( $carousel.is( '.mpc-carousel--stretched' ) ) {
			...
			$nav_items.first().css( 'margin-left', ( _window_size - _stretched_size ) * -0.5 );
			$nav_items.last().css( 'margin-right', ( _window_size - _stretched_size ) * -0.5 );
		} else if( $carousel.parents( '.vc_row' ).attr( 'data-vc-stretch-content' ) == 'true' ) {
			$nav_items.addClass( 'mpc-nav--stretched' );
		}
	}
...
(function( $ ) {
	...
	$toggle_rows.each( function() {
		var $toggle_row   = $( this ),
		    ...
		    $toggable_row = $( '.vc_row[data-row-id="' + _row_id + '"]' ),
		    _loaded       = false,
		    _row_height;
...
(function( $ ) {
	...
	$rows.each( function() {
		var $row                    = $( this ),
		    $prev_row               = $row.prevAll( '.vc_row' ).first(),
		    $next_row               = $row.nextAll( '.vc_row' ).first(),
...
( function( $ ) {
	...
	function responsive( $tabs ) {
		var _is_12_col = $tabs.parents( '.vc_column_container' ).is( '.vc_col-sm-12' );
...
( function( $ ) {
	mpc_init_lightbox = function( $element, _is_gallery ) {
		...
		if( $.fn.prettyPhoto && _vendor == 'prettyphoto' ) {
			...
			var _type = /\.(gif|jpg|jpeg|tiff|png)/i.test( $lightbox.attr( 'href' ) ) ? 'image' : 'iframe',
				_atts;
